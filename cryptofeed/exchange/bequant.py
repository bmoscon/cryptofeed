...
class Bequant(Feed):
    ...
    async def _trades(self, msg: dict, ts: float):
        """
        "params": {
            "data": [
            {
    ...
    async def _candles(self, msg: dict, ts: float):
        """
        {
            "jsonrpc": "2.0",
    ...
    async def message_handler(self, msg: str, conn: AsyncConnection, ts: float):
        ...
        if 'params' in msg and 'sequence' in msg['params']:
            ...
            if pair in ic(self.seq_no):
                if self.seq_no[pair] + 1 != ic(msg['params']['sequence']):
