...
from cryptofeed.defines import ASK, BEQUANT, BID, HITBTC, L2_BOOK, ORDER_INFO, ACC_BALANCES, ACC_TRANSACTIONS, TICKER, CANDLES, TRADES
...
def main():
    ...
    f.add_feed(HITBTC, channels=[TICKER], symbols=['ADA-USDT'], callbacks={TICKER: TickerCallback(ticker)})
    f.add_feed(BEQUANT, channels=[L2_BOOK], symbols=['ALGO-USDT'], callbacks={L2_BOOK: (book)})
    f.add_feed(HITBTC, channels=[L2_BOOK], symbols=['ALGO-USDT'], callbacks={L2_BOOK: (book)})
    f.add_feed(BEQUANT, channels=[CANDLES], symbols=['ETH-USDT'], callbacks={CANDLES: candles_callback})
    f.add_feed(HITBTC, channels=[CANDLES], symbols=['ETH-USDT'], callbacks={CANDLES: candles_callback})
    f.add_feed(BEQUANT, channels=[TRADES], symbols=['ETH-USDT'], callbacks={TRADES: trade})
    f.add_feed(HITBTC, channels=[TRADES], symbols=['ETH-USDT'], callbacks={TRADES: trade})
    ...
    f.add_feed(HITBTC, subscription={ORDER_INFO: ['BTC-USDT', 'ETH-USDC']}, callbacks={ORDER_INFO: order})
