...
async def book(feed, pair, book, timestamp):
    print(
        f'Timestamp: {timestamp} Feed: {feed} Pair: {pair} Book Bid Size is {len(book[BID])} Ask Size is {len(book[ASK])}')
...
def main():
    ...
    config = {TRADES: ['BTC-28JUN19', "ETH-27SEP19",
                       "BTC-PERPETUAL"], TICKER: ['ETH-27SEP19']}
    f.add_feed(Deribit(config=config, callbacks={
               TICKER: TickerCallback(ticker), TRADES: TradeCallback(trade)}))

    f.add_feed(Deribit(pairs=[
               'BTC-PERPETUAL'], channels=[L2_BOOK], callbacks={L2_BOOK: BookCallback(book)}))
    f.run()
