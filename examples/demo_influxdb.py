...
def main():
    ...
    f.add_feed(
        Coinbase(
            channels=[TRADES], symbols=["BTC-USD"], callbacks={TRADES: TradeInflux(INFLUX_ADDR, ORG, BUCKET, TOKEN)}
        )
    )
    f.add_feed(
        Coinbase(
            channels=[L2_BOOK], symbols=["BTC-USD"], callbacks={L2_BOOK: BookInflux(INFLUX_ADDR, ORG, BUCKET, TOKEN)}
        )
    )
    f.add_feed(
        Coinbase(
            channels=[TICKER], symbols=["BTC-USD"], callbacks={TICKER: TickerInflux(INFLUX_ADDR, ORG, BUCKET, TOKEN)}
        )
    )
    f.add_feed(
        Binance(
