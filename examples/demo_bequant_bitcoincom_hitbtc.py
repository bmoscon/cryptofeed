...
from cryptofeed.callback import BalancesCallback, TickerCallback, TransactionsCallback
...
def main():
    ...
    f.add_feed(
        BEQUANT, channels=[CANDLES], candle_interval="30m", symbols=["ETH-USDT"], callbacks={CANDLES: candles_callback}
    )
    f.add_feed(
        HITBTC, channels=[CANDLES], candle_interval="30m", symbols=["NEO-USDT"], callbacks={CANDLES: candles_callback}
    )
    ...
    f.add_feed(
        BEQUANT, timeout=-1, channels=[BALANCES], symbols=["XLM-USDT"], callbacks={BALANCES: BalancesCallback(balances)}
    )
    f.add_feed(
        HITBTC, timeout=-1, channels=[BALANCES], symbols=["ADA-USDT"], callbacks={BALANCES: BalancesCallback(balances)}
    )
    f.add_feed(
        BEQUANT,
        timeout=-1,
