...
def main():
    ...
    f = FeedHandler(raw_message_capture=AsyncFileCallback(DIR_DATA), handler_enabled=True)
    f.add_feed(Binance(max_depth=5, pairs=list_symbol_binance, channels=[TRADES, TICKER]))
    f.add_feed(BinanceFutures(max_depth=5, pairs=list_symbol_binance_future, channels=[TRADES, TICKER]))
    #f.add_feed(BinanceDelivery(max_depth=5, pairs=list_symbol_binance_delivery, channels=[L2_BOOK]))
    f.add_feed(BinanceDelivery(max_depth=5, pairs=list_symbol_binance_delivery, channels=[TRADES, TICKER]))
    f.run()
