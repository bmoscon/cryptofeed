...
def main():
    ...
    # f.add_feed(Poloniex(channels=['USDT-BTC'], callbacks={L3_BOOK: BookCallback(book), TRADES: TradeCallback(trade)}))
    ...
    # f.add_feed(HitBTC(channels=[L3_BOOK], pairs=['BTC-USD'], callbacks={L3_BOOK: BookCallback(book)}))
    # f.add_feed(EXX())
    # f.add_feed(Bitstamp(channels=[L3_BOOK], pairs=['BTC-USD'], callbacks={L3_BOOK: BookCallback(book)}))
    f.run()
