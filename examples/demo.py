...
from cryptofeed.exchanges import (Binance, BinanceUS, BinanceFutures, Bitfinex, Bitflyer, Bitget, Bithumb, Bitstamp,
                                  Bybit, Coinbase, CryptoDotCom, Deribit, Gateio, Huobi, HuobiSwap,
                                  IndependentReserve, Kraken, KrakenFutures, KuCoin, OKX, Phemex, Poloniex, Upbit)
...
def main():
    ...
    f.add_feed(Bybit(symbols=['BTC-USDT-PERP'], channels=[INDEX, FUNDING, OPEN_INTEREST, LIQUIDATIONS], callbacks={OPEN_INTEREST: oi, INDEX: index, FUNDING: funding, LIQUIDATIONS: liquidations}))
    f.add_feed(Bybit(candle_closed_only=True, symbols=['BTC-USDT-PERP'], channels=[CANDLES, TRADES, L2_BOOK], callbacks={CANDLES: candle_callback, TRADES: trade, L2_BOOK: book}))
