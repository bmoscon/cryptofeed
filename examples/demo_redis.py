...
from cryptofeed.exchanges import Bitfinex, Bybit, Coinbase, Gemini, Binance


def main():
    config = {'log': {'filename': 'redis-demo.log', 'level': 'INFO'}, 'backend_multiprocessing': True}
    f = FeedHandler(config=config)
    f.add_feed(Bybit(channels=[TRADES, FUNDING, OPEN_INTEREST], symbols=['BTC-USDT-PERP'], callbacks={TRADES: TradeRedis(), FUNDING: FundingRedis(), OPEN_INTEREST: OpenInterestRedis()}))
