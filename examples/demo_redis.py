...
from cryptofeed.exchanges import Binance, Bitfinex, Bitmex, Coinbase, Gemini
...
def main():
    ...
    f.add_feed(
        Bitmex(
            ...
            callbacks={TRADES: TradeRedis(), FUNDING: FundingRedis(), OPEN_INTEREST: OpenInterestRedis()},
        )
    )
    ...
    f.add_feed(
        Binance(
            max_depth=10, symbols=["BTC-USDT"], channels=[L2_BOOK], callbacks={L2_BOOK: BookRedis(snapshots_only=True)}
        )
    )
    f.add_feed(
        Binance(max_depth=10, symbols=["BTC-USDT"], channels=[L2_BOOK], callbacks={L2_BOOK: BookSnapshotRedisKey()})
    )
